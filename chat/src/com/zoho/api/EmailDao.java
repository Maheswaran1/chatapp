...
public class EmailDao implements EmailInterface{	
	...
	public ArrayList<Email> getVerifedEmails(String userId,boolean status) {
		...
		HashMap<String,Object> condition2 = new HashMap<String,Object>();
		condition1.put("UserId",userId);
		condition2.put("Verified", status);
		condition.put("AND", condition1);
		condition.put("", condition2);
		String [] column = {"*"};
		...
		try {
			ExcuteQuery queryObject = new ExcuteQuery();
			ArrayList<ObjectFactory> emailList =queryObject.selectMultiple(tableName, column, condition2);
			ArrayList<Email> emailListOriginal = new ArrayList<Email>();
			for(ObjectFactory object : emailList) {
				Email email = (Email) object;
				emailListOriginal.add(email);
			}
			return emailListOriginal;
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
